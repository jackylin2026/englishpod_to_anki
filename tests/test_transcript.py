...
BESIDE = "englishpod_C0108.transcript.md"


def wrote_note(lesson: Path, tmp_path: Path) -> Path:
    """The note a build wrote for a lesson, out of its directory in the build tree."""
    (path,) = built_dir(lesson, tmp_path).glob("englishpod_*.json")
    return path
...
def test_a_lesson_the_transcript_covers_is_written_beside_its_markdown(
    lesson: Path, print_transcript: Path, run_cli, tmp_path: Path
) -> None:
    ...
    beside = built_dir(lesson, tmp_path) / BESIDE
...
def test_a_lesson_the_print_agrees_with_is_not_reported(
    ...
    written = markdown_file(lesson, tmp_path).read_text(encoding="utf-8")
    ...
    assert markdown_file(lesson, tmp_path).read_text(encoding="utf-8") == written
...
def test_a_lesson_the_print_disagrees_with_is_reported_and_left_alone(
    ...
    run_cli("preprocess", lesson)
    corrected = markdown_file(lesson, tmp_path).read_text(encoding="utf-8").replace(
        "## Key Vocabulary", "A: And the auditors?\n\n## Key Vocabulary", 1
    )
    markdown_file(lesson, tmp_path).write_text(corrected, encoding="utf-8")

    result = run_cli("preprocess", lesson, "--print-transcript", print_transcript)

    assert result.returncode == 0, result.stderr
    assert f"C0108: the print transcript's dialogue differs from the lesson's; see {built_dir(lesson, tmp_path) / BESIDE}" in result.stderr
    assert markdown_file(lesson, tmp_path).read_text(encoding="utf-8") == corrected
...
def test_the_card_built_from_a_reported_lesson_is_the_lesson_s_own(
    ...
    room_build = built_dir(room, tmp_path)
    room_build.mkdir(parents=True, exist_ok=True)
    shutil.copy(markdown_file(lesson, tmp_path), room_build / SAMPLE)

    result = run_cli("build", room, "--offline")

    assert result.returncode == 0, result.stderr
    # The lesson's own words, which the print's own file says otherwise about:
    # the print splits the sentence, and has no entry for `immaculate`.
    sentences = json.loads(
        wrote_note(room, tmp_path).read_text(encoding="utf-8")
    )["fields"]["Sentences"]
    assert "{{c1::immaculate}}" in sentences
    assert "entry-level" in sentences


def test_a_lesson_the_transcript_does_not_cover_is_processed_unreported(
    scanned_lesson: Path, print_transcript: Path, run_cli, tmp_path: Path
) -> None:
    """The print holds no dialogue for it, so there is nothing to compare."""
    built = built_dir(scanned_lesson, tmp_path)
    built.mkdir(parents=True, exist_ok=True)
    (built / "englishpod_C0109.md").write_text(
    ...
    assert result.returncode == 0, result.stderr
    assert "differs" not in result.stderr
    assert sorted(path.name for path in built_dir(scanned_lesson, tmp_path).glob("*.md")) == ["englishpod_C0109.md"]
...
def test_the_transcript_file_is_not_the_lesson_markdown(
    ...
    sentences = json.loads(
        wrote_note(lesson, tmp_path).read_text(encoding="utf-8")
    )["fields"]["Sentences"]
    assert "{{c1::immaculate}}" in sentences


def test_a_transcript_file_does_not_stand_in_for_a_deleted_markdown(
    corpus: Path, print_transcript: Path, run_cli, tmp_path: Path
) -> None:
    ...
    written = built_dir(lesson, tmp_path) / "englishpod_B0110.md"
    written.unlink()

    result = run_cli("preprocess", corpus, "--print-transcript", print_transcript, "--force")
    ...
    # The transcript's file is written again for the lesson, under the name of
    # the Markdown the lesson has.
    assert (written.parent / "englishpod_B0110.transcript.md").is_file()
...
def test_a_transcript_file_written_by_hand_is_left_alone(
    ...
    run_cli("preprocess", lesson, "--print-transcript", print_transcript)
    (built_dir(lesson, tmp_path) / BESIDE).write_text("## Dialogue\n\nA: My own reading.\n", encoding="utf-8")

    result = run_cli("preprocess", lesson, "--print-transcript", print_transcript)

    assert result.returncode == 0, result.stderr
    assert (built_dir(lesson, tmp_path) / BESIDE).read_text(encoding="utf-8") == "## Dialogue\n\nA: My own reading.\n"

    forced = run_cli("preprocess", lesson, "--print-transcript", print_transcript, "--force")

    assert forced.returncode == 0, forced.stderr
    assert (built_dir(lesson, tmp_path) / BESIDE).read_text(encoding="utf-8") == EXPECTED_TRANSCRIPT
...
def test_a_run_over_a_corpus_says_what_it_checked_and_what_disagreed(
    ...
    # Each lesson's transcript file is named for the Markdown it was checked
    # against, which is the one the migration put there.
    assert (built_dir(corpus / BATCH / "0110", tmp_path) / "englishpod_B0110.transcript.md").is_file()
    kettle = built_dir(corpus / BATCH / "0111", tmp_path) / "englishpod_F0111.transcript.md"
    assert kettle.read_text(encoding="utf-8") == (
        "## Dialogue\n\nA: The kettle is broken once more.\n\n"
        "B: I will buy a new one tomorrow.\n"
    )
    assert f"{kettle}" in result.stderr
